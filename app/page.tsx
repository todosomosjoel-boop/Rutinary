...
function Dashboard() {
  ...
  const data = useMemo(() => {
    ...
    const currentBook = readings.find(r => !r.completed) || readings[0];
    const pagesToday = currentBook ? readJson<any>(`ritmo_readlog_${user.username}_${currentBook.id}_${today}`, {pages:0}).pages || 0 : 0;
  ...
  const checks = [data.exercise.completed, data.pagesToday>=goals.readingPages, data.sleep.hours>=goals.sleepHours, data.water>=goals.hydrationMl, data.foods.length>0];
